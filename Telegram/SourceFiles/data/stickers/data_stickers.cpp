...
std::vector<not_null<DocumentData*>> Stickers::getListByEmoji(
	...
	// LoogriGram: upstream appended "stickers_premium_by_emoji_num" premium
	// stickers after these, drawn locked. Premium stickers are not offered.
	return result
		| ranges::views::transform(&StickerWithDate::document)
		| ranges::views::filter([](not_null<DocumentData*> document) {
