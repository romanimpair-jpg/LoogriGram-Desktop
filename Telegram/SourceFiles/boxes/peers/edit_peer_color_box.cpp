...
void EditPeerColorSection(
	...
	// gift's colour. Collectible colours are not parsed at all now, so
	// there is nothing to choose or clear here but the plain palette.
...
void SetupPeerColorSample(
	...
	// LoogriGram: a worn collectible gift status was drawn next to the
	// profile colour sample here, and took over its colours.
	rpl::combine(
		button->widthValue(),
		rpl::duplicate(label),
		rpl::duplicate(colorIndexValue),
